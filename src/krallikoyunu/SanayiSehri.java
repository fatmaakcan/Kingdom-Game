...
public class SanayiSehri extends Sehir {
    ...
    public SanayiSehri(String ad, double uretimKapasitesi, int gelisimSeviyesi, int orduSeviyesi, int guvenlikSeviyesi, int halkMemnuniyeti, double vergiOrani) { //SanayiSehri'nin üst sınıfı (parent class) olan Sehir sınıfının özellikleri inheritance (kalıtım) ilkesi ile bu sınıfta tanımlanmıştır.
    ...
    public void turSonuUretimHesapla(Krallik krallik) {
        double altinUretim = 0.6 * (super.getGelisimSeviyesi() + super.getUretimKapasitesi()); //SanayiSehri, TicaretSehrinden az TarimSehri'nden fazla olacak şekilde bir altın üretimi yaptığından dolayı ilgili şehrin gelişim seviyesi ve üretim kapasitesinin toplamının 0.6 katsayısı ile çarpılmasıyla altinUretimi hesaplanmaktadır.
        double yiyecekUretim = 0.4 * (super.getGelisimSeviyesi() + super.getUretimKapasitesi()); //Gelişim ve üretim kapasitesinin toplamının 0.4 katsayısı ile çarpılmasıyla yiyecekUretimi hesaplanmaktadır.
        double altinVergisi = altinUretim * this.getVergiOrani();//Yapılan altınUretimi, ilgili şehrin belirlenen vergiOrani ile çarpılarak altinVergisi hesaplanmaktadır.
        ...
        if (baskinGerceklesiyorMu()) {//Bu if bloğu baskın gerçekleşip gerçekleşmediğinin kontrolünü yapmaktadır.
            double altinKalan = altinUretim - 20;//Eğer ki baskın yapıldıysa, ilgili şehirde üretilen altinUretim'i baskından dolayı bir miktar kayba uğrar ve kalanı da altinKalan adlı değişkende saklanır.
        ...
        super.turSonuUretimHesapla(krallik);//SanayiSehri'nin parent class'ı olan Sehir sınıfındaki turSonuUretimHesapla() metodu çağırılmaktadır.Bu metot, şehir bazlı vergi oranına bağı olarak ilgili şehrin halk memnuniyetini güncellemektedir.
        if (altinUretim >= 50 || yiyecekUretim >= 50) {//altinUretimi ve yiyecekUretimi'nin 50'ye eşit ya da fazla olması durumunda bu if bloğu çalışmaktadır.
