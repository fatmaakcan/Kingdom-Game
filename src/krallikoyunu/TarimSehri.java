...
public class TarimSehri extends Sehir {

    public TarimSehri(String ad, double uretimKapasitesi, int gelisimSeviyesi, int orduSeviyesi, int guvenlikSeviyesi, int halkMemnuniyeti, double vergiOrani) { //TarimSehri'nin üst sınıfı (parent class) olan Sehir sınıfının özellikleri inheritance (kalıtım) ilkesi ile bu sınıfta tanımlanmıştır.
    ...
    public void turSonuUretimHesapla(Krallik krallik) {
        double altinUretim = 0.2 * (super.getGelisimSeviyesi() + super.getUretimKapasitesi());//Gelişim seviyesi ve üretim kapasitesinin toplamının 0.2 katsayısı ile çarpılmasıyla altinUretimi hesaplanmaktadır.
        double yiyecekUretim = 0.8 * (super.getGelisimSeviyesi() + super.getUretimKapasitesi());//TarimSehri yiyecek üretimi odaklı bir üretim yaptığından dolayı ilgili şehrin gelişim seviyesi ve üretim kapasitesinin toplamının 0.8 katsayısı ile çarpılmasıyla yiyecekUretimi hesaplanmaktadır.
        ...
        if (baskinGerceklesiyorMu()) {//Bu if bloğu baskın gerçekleşip gerçekleşmediğinin kontrolünü yapmaktadır.
            double altinKalan = altinUretim - 10;//Eğer ki baskın yapıldıysa, ilgili şehirde üretilen altinUretim'i baskından dolayı bir miktar kayba uğrar ve kalanı da altinKalan adlı değişkende saklanır.
            double yiyecekKalan = yiyecekUretim - 30;// Aynı şekilde, ilgili şehirde üretilen yiyecekUretim'i baskından dolayı bir miktar kayba uğrar ve kalanı da yiyecekKalan adlı bir değişkende saklanır.
            if (altinKalan <= 0 && yiyecekKalan <= 0) {//Bu if bloğu, baskından dolayı üretilen kaynakların her ikisinin de bitip bitmediğini veya negatife düşüp düşmediğinin kontrolünü sağlar.
                ...
                if (altinKalan <= 0) {//Bu if bloğu, baskın sonrası altinKalan'ın sıfır veya negatife düşmesi durumunu kontrol etmektedir.
            ...
            if (this.getHalkMemnuniyeti() - 1 < 0) {//Baskın gerçekleşmesi durumunda ilgili şehrin memnuniyet seviyesi 1 azalmaktadır ve bu azalmanın negatif bir sayıya neden olup olmayacağı kontrol edilmektedir.
                ...
                this.setHalkMemnuniyeti(0);//halkMemnuniyeti'nin negatif bir değere düşmesini engellemek için minimum sınır(0)'a sabitlenmektedir.
            ...
            krallik.setToplamAltin(krallik.getToplamAltin() + altinVergisi);//turSonuUretimHesapla() metodunun başında hesaplanan altinVergisi krallığın altınına eklenmektedir.
            krallik.setToplamYemek(krallik.getToplamYemek() + yiyecekVergisi);//turSonuUretimHesapla() metodunun başında hesaplanan yiyecekVergisi krallığın yemek miktarına eklenmektedir.
        }

        super.turSonuUretimHesapla(krallik);//TarimSehri'nin parent class'ı olan Sehir sınıfındaki turSonuUretimHesapla() metodu çağırılmaktadır.Bu metot, şehir bazlı vergi oranına bağı olarak ilgili şehrin halk memnuniyetini güncellemektedir.
        if (yiyecekUretim >= 50) {//yiyecekUretimi'nin 50'ye eşit ya da fazla olması durumunda bu if bloğu çalışmaktadır.
